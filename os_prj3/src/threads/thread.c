...
thread_set_priority (int new_priority) 
{
  ...
  if(pre_priority > new_priority) thread_yield();
}
...
/*prj3*/
bool
compare_priority(const struct list_elem *l, const struct list_elem *r, void *aux){
...
update_priority(void){
  ...
  while(e != list_end(&all_list)){
    ...
    t->priority = ((PRI_MAX * FRACTION) - (t->recent_cpu/4) - (2 * t->nice) * FRACTION)/FRACTION;
...
thread_set_nice (int new_nice) 
{
  ...
  t->priority = ((PRI_MAX * FRACTION) - (t->recent_cpu/4) - (2 * (new_nice * FRACTION)))/FRACTION;
  if(t->priority > PRI_MAX) t->priority = PRI_MAX;
  else if(t->priority < PRI_MIN) t->priority = PRI_MIN;
}
