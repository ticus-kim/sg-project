...
process_execute (const char *file_name) 
{
  ...
  char *fn_copy_1 = (char *)malloc(sizeof(char) * (1 + strlen(file_name)));
  char *ptr, *next_ptr;
  
  strlcpy(fn_copy_1,file_name,strlen(file_name)+1);
  ptr = strtok_r(fn_copy_1," ",&next_ptr);

  if(filesys_open(ptr) == NULL) {
	return -1;
  }

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (ptr, PRI_DEFAULT, start_process, fn_copy);
          ...
          if (validate_segment (&phdr, file)) 
            {
              ...
              if (!load_segment (file, file_page, (void *) mem_page,
                                 read_bytes, zero_bytes, writable, argv[0]))
                goto done;
            }
          else
            goto done;
...
void argument_stack(char** argv, int argc, void **esp)
{
   ...
   int padding = total_len % 4;
   if(padding != 0) *esp -= 4-padding;
   ...
   **(uint32_t **)esp = 0;
   ...
   for(int i = argc; i >= 0; i--){
      *esp = *esp - 4;
	  ...
	  if(i == argc) **(uint32_t **) esp = 0;
	  else **(uint32_t **) esp = addr[i];
   }
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      vme->vaddr = upage;
      vme->is_loaded = false;
      vme->filename = (char *)malloc(sizeof(char)*strlen(file_name)+1);
      strlcpy(vme->filename, file_name, strlen(file_name)+1);
...
bool verify_stack (void *fault_addr, void *esp){
  void *max = PHYS_BASE - 8 * 1024 * 1024;
	return is_user_vaddr(pg_round_down(fault_addr)) && fault_addr >= esp - 32 && fault_addr >= max;
}
