...
struct vm_entry *check_addr(void *addr, void *esp){
	if((void * )0x8048000 > addr || (void * ) 0xc0000000 <= addr) exit(-1);
...
void check_valid_buffer(void *buffer, unsigned size, void *esp, bool to_write)
{
	...
	while(ptr<buffer+size){
		struct vm_entry *vme = check_addr(ptr, esp);
		if(!vme || !(vme->writable)) exit(-1);
		ptr += PGSIZE;
	}
}
...
void check_valid_str(void *str, void *esp){
	...
	while(ptr < str + size){
		vme = check_addr(ptr, esp);
		if(!vme) exit(-1);
		ptr += PGSIZE;
	}
}

void check_valid_str_by_size(void *str, unsigned size, void *esp){
	for(int i=0; i<(int)size; i++){
		struct vm_entry *vme = check_addr((void *)(str), esp);
		if(!vme) exit(-1);
	}
}
...
syscall_handler (struct intr_frame *f UNUSED) 
{
   ...
   switch(*(uint32_t*)f->esp) {
	  ...
	  case SYS_EXEC:
	     check_user_vaddr(f->esp+4);
		 check_valid_str((void *)(f->esp + 4), f->esp);
	  ...
	  case SYS_READ:
		 ...
		 check_valid_str_by_size((void*)*(uint32_t*)(f->esp+8), (unsigned)*(uint32_t*)(f->esp+12), f->esp);
